...
; Maybe add a label here for the prio loop
	LDR		R1, [R0] 		   ; R1 = new RunPt
	LDR		R2, [R1,#12]	   ; R2 = RunPt->bi
	CMP		R2, #0
	BNE		Loop			   ; If R2 is not equal to zero that means its blocked (Due to lab 5 deadlock maybe I can set its prio to 0 but . Ask professor)

	; do prio check here (R0 is used for address of RunPtAddr, R1 is used for RunPt, R2 is a counter for the loop, R3 is used for prio of inital thread to be compared, and R4 is the RunPt->next
...
Label:
	...
	ADD		R2, #1			   ; R2 = R2 + 1
	CMP		R2, #2
	BNE		Label			   ; Branch means we continue looping to compare to the next thread

	LDR		R1, [R0]		   ; R1 = RunPt(Because we have a new RunPt we need to put this in R1 again) We have exited the loop
	LDR		R4, [R1,#8]		   ; R4 = RunPt->next
	STR 	R4, [R0]		   ; RunPt = RunPT->next (We have come full circle)


	LDR	    R0, RunPtAddr 		; R0 won't have the right address so you have to do reload it I think
